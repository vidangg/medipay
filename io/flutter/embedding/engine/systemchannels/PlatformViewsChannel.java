...
public class PlatformViewsChannel {
    ...
    class AnonymousClass1 implements MethodChannel.MethodCallHandler {
        ...
        private void create(MethodCall methodCall, MethodChannel.Result result) {
            ...
            try {
                if (z2) {
                    PlatformViewsChannel.this.handler.createForPlatformViewLayer(new PlatformViewCreationRequest(((Integer) map.get(TtmlNode.ATTR_ID)).intValue(), (String) map.get("viewType"), AudioStats.AUDIO_AMPLITUDE_NONE, AudioStats.AUDIO_AMPLITUDE_NONE, AudioStats.AUDIO_AMPLITUDE_NONE, AudioStats.AUDIO_AMPLITUDE_NONE, ((Integer) map.get("direction")).intValue(), PlatformViewCreationRequest.RequestedDisplayMode.HYBRID_ONLY, wrap));
                    result.success(null);
                    return;
                }
                ...
                if (map.containsKey(TtmlNode.LEFT)) {
                    d = ((Double) map.get(TtmlNode.LEFT)).doubleValue();
                }
