    ...
    public static InputImage convertMlImagetoInputImage(MlImage mlImage) {
        ...
        if (storageType != 1) {
            ...
            if (storageType == 2) {
                ...
                if (imageFormat != 4) {
                    num = imageFormat != 5 ? null : Integer.valueOf(InputImage.IMAGE_FORMAT_YV12);
                } else {
                    num = 17;
                }
                if (num != null) {
                ...
                if (extract2.getFormat() == 256) {
                    limit = extract2.getPlanes()[0].getBuffer().limit();
                } else {
                    limit = (extract2.getPlanes()[0].getBuffer().limit() * 3) / 2;
                }
                zza(extract2.getFormat(), 5, SystemClock.elapsedRealtime(), mlImage.getHeight(), mlImage.getWidth(), limit, mlImage.getRotation());
