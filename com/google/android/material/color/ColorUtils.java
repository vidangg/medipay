...
final class ColorUtils {
    ...
    public static String hexFromInt(int i) {
        ...
        return String.format("#%02x%02x%02x", Integer.valueOf(redFromInt), Integer.valueOf(greenFromInt(i)), Integer.valueOf(blueFromInt));
    }
    ...
    public static int intFromXyzComponents(float f, float f2, float f3) {
        ...
        return intFromRgb(Math.max(Math.min(255, Math.round(delinearized(f7) * 255.0f)), 0), Math.max(Math.min(255, Math.round(delinearized(f8) * 255.0f)), 0), Math.max(Math.min(255, Math.round(delinearized(f9) * 255.0f)), 0));
    }
