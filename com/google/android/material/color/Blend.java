...
final class Blend {
    ...
    private static float rotationDirection(float f, float f2) {
        ...
        return (abs > abs2 || abs > abs3) ? (abs2 > abs || abs2 > abs3) ? ((double) f5) >= AudioStats.AUDIO_AMPLITUDE_NONE ? 1.0f : -1.0f : ((double) f4) >= AudioStats.AUDIO_AMPLITUDE_NONE ? 1.0f : -1.0f : ((double) f3) >= AudioStats.AUDIO_AMPLITUDE_NONE ? 1.0f : -1.0f;
    }
}
