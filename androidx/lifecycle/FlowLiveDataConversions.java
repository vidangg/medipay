...
public final class FlowLiveDataConversions {
    ...
    public static final <T> LiveData<T> asLiveData(Flow<? extends T> flow, CoroutineContext context, long j) {
        Intrinsics.checkNotNullParameter(flow, "<this>");
        Intrinsics.checkNotNullParameter(context, "context");
        FlutterFirebaseRemoteMessageLiveData flutterFirebaseRemoteMessageLiveData = (LiveData<T>) CoroutineLiveDataKt.liveData(context, j, new FlowLiveDataConversions$asLiveData$1(flow, null));
        if (flow instanceof StateFlow) {
            if (ArchTaskExecutor.getInstance().isMainThread()) {
                flutterFirebaseRemoteMessageLiveData.setValue(((StateFlow) flow).getValue());
            } else {
                flutterFirebaseRemoteMessageLiveData.postValue(((StateFlow) flow).getValue());
            }
        }
