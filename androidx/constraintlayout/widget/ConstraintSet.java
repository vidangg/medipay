...
public class ConstraintSet {
    ...
    */
    public static void parseDimensionConstraints(Object data, TypedArray a, int attr, int orientation) {
        ...
        int i2 = 0;
        if (i == 5) {
            dimensionPixelSize = a.getDimensionPixelSize(attr, 0);
        } else {
            dimensionPixelSize = a.getInt(attr, 0);
    ...
    public static void parseDimensionRatioString(ConstraintLayout.LayoutParams params, String value) {
        ...
        if (value != null) {
            ...
            int indexOf2 = value.indexOf(WIDTH_MIN);
            try {
    ...
    public void clone(ConstraintSet set) {
        ...
        for (Integer num : set.mConstraints.keySet()) {
            Constraint constraint = set.mConstraints.get(num);
            if (constraint != null) {
                this.mConstraints.put(num, constraint.m316clone());
            }
        }
    }
    ...
    */
            ...
            while (eventType != 1) {
                if (eventType != 0) {
                    ...
                    if (eventType == 2) {
                        ...
                        switch (c) {
                            case 0:
                                fillFromAttributeList = fillFromAttributeList(context, Xml.asAttributeSet(parser), false);
                                constraint = fillFromAttributeList;
                                break;
                            case 1:
                                fillFromAttributeList = fillFromAttributeList(context, Xml.asAttributeSet(parser), true);
                                constraint = fillFromAttributeList;
                                break;
                            case 2:
                                fillFromAttributeList = fillFromAttributeList(context, Xml.asAttributeSet(parser), false);
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            switch (overrideMapToConstant.get(index)) {
                ...
                case 55:
                    ...
                    break;
                case WIDTH_MAX /* 56 */:
                    delta.add(WIDTH_MAX, a.getDimensionPixelSize(index, c.layout.widthMax));
                    break;
                case HEIGHT_MAX /* 57 */:
                    delta.add(HEIGHT_MAX, a.getDimensionPixelSize(index, c.layout.heightMax));
                    break;
                case WIDTH_MIN /* 58 */:
                    delta.add(WIDTH_MIN, a.getDimensionPixelSize(index, c.layout.widthMin));
                    break;
                case HEIGHT_MIN /* 59 */:
                    delta.add(HEIGHT_MIN, a.getDimensionPixelSize(index, c.layout.heightMin));
                    break;
                case 60:
                    delta.add(60, a.getFloat(index, c.transform.rotation));
                    break;
                case CIRCLE_RADIUS /* 62 */:
                    delta.add(CIRCLE_RADIUS, a.getDimensionPixelSize(index, c.layout.circleRadius));
                    break;
                case 63:
                ...
                case 67:
                    ...
                    break;
                case PROGRESS /* 68 */:
                    delta.add(PROGRESS, a.getFloat(index, c.propertySet.mProgress));
                    break;
                case WIDTH_PERCENT /* 69 */:
                    delta.add(WIDTH_PERCENT, a.getFloat(index, 1.0f));
                    break;
                case HEIGHT_PERCENT /* 70 */:
                    delta.add(HEIGHT_PERCENT, a.getFloat(index, 1.0f));
                    break;
                case 71:
                    Log.e(TAG, "CURRENTLY UNSUPPORTED");
                    break;
                case BARRIER_DIRECTION /* 72 */:
                    delta.add(BARRIER_DIRECTION, a.getInt(index, c.layout.mBarrierDirection));
                    break;
                case BARRIER_MARGIN /* 73 */:
                    delta.add(BARRIER_MARGIN, a.getDimensionPixelSize(index, c.layout.mBarrierMargin));
                    break;
                case CONSTRAINT_REFERENCED_IDS /* 74 */:
                    delta.add(CONSTRAINT_REFERENCED_IDS, a.getString(index));
                    break;
                case BARRIER_ALLOWS_GONE_WIDGETS /* 75 */:
                    delta.add(BARRIER_ALLOWS_GONE_WIDGETS, a.getBoolean(index, c.layout.mBarrierAllowsGoneWidgets));
                    break;
                case 76:
                    delta.add(76, a.getInt(index, c.motion.mPathMotionArc));
                    break;
                case CONSTRAINT_TAG /* 77 */:
                    delta.add(CONSTRAINT_TAG, a.getString(index));
                    break;
                case VISIBILITY_MODE /* 78 */:
                    delta.add(VISIBILITY_MODE, a.getInt(index, c.propertySet.mVisibilityMode));
                    break;
                case MOTION_STAGGER /* 79 */:
                    delta.add(MOTION_STAGGER, a.getFloat(index, c.motion.mMotionStagger));
                    break;
                case 80:
                    delta.add(80, a.getBoolean(index, c.layout.constrainedWidth));
                    break;
                case CONSTRAINED_HEIGHT /* 81 */:
                    delta.add(CONSTRAINED_HEIGHT, a.getBoolean(index, c.layout.constrainedHeight));
                    break;
                case ANIMATE_CIRCLE_ANGLE_TO /* 82 */:
                    delta.add(ANIMATE_CIRCLE_ANGLE_TO, a.getInteger(index, c.motion.mAnimateCircleAngleTo));
                    break;
                case TRANSFORM_PIVOT_TARGET /* 83 */:
                    delta.add(TRANSFORM_PIVOT_TARGET, lookupID(a, index, c.transform.transformPivotTarget));
                    break;
                case QUANTIZE_MOTION_STEPS /* 84 */:
                    delta.add(QUANTIZE_MOTION_STEPS, a.getInteger(index, c.motion.mQuantizeMotionSteps));
                    break;
                case QUANTIZE_MOTION_PHASE /* 85 */:
                    delta.add(QUANTIZE_MOTION_PHASE, a.getFloat(index, c.motion.mQuantizeMotionPhase));
                    break;
                ...
                case BASELINE_MARGIN /* 93 */:
                    delta.add(BASELINE_MARGIN, a.getDimensionPixelSize(index, c.layout.baselineMargin));
                    break;
                case GONE_BASELINE_MARGIN /* 94 */:
                    delta.add(GONE_BASELINE_MARGIN, a.getDimensionPixelSize(index, c.layout.goneBaselineMargin));
                    break;
                ...
                case LAYOUT_WRAP_BEHAVIOR /* 97 */:
                    delta.add(LAYOUT_WRAP_BEHAVIOR, a.getInt(index, c.layout.mWrapBehavior));
                    break;
                ...
                case GUIDELINE_USE_RTL /* 99 */:
                    delta.add(GUIDELINE_USE_RTL, a.getBoolean(index, c.layout.guidelineUseRtl));
                    break;
            }
        }
    }
    ...
    public static void setDeltaValue(Constraint c, int type, float value) {
        ...
        if (type == MOTION_STAGGER) {
            c.motion.mMotionStagger = value;
            return;
        }
        if (type == QUANTIZE_MOTION_PHASE) {
    ...
    public static void setDeltaValue(Constraint c, int type, int value) {
        ...
        if (type == CIRCLE_RADIUS) {
            c.layout.circleRadius = value;
            return;
        }
        if (type == BARRIER_DIRECTION) {
            c.layout.mBarrierDirection = value;
            return;
        }
        if (type != BARRIER_MARGIN) {
            switch (type) {
                case 2:
    ...
    public static void setDeltaValue(Constraint c, int type, String value) {
        ...
        if (type == CONSTRAINT_REFERENCED_IDS) {
            c.layout.mReferenceIdString = value;
            c.layout.mReferenceIds = null;
        } else if (type == CONSTRAINT_TAG) {
            c.layout.mConstraintTag = value;
        } else if (type != UNUSED) {
            if (type == QUANTIZE_MOTION_INTERPOLATOR_STR) {
    ...
    public static void setDeltaValue(Constraint c, int type, boolean value) {
        ...
        if (type == BARRIER_ALLOWS_GONE_WIDGETS) {
        ...
        if (type != UNUSED) {
            ...
            } else if (type == CONSTRAINED_HEIGHT) {
