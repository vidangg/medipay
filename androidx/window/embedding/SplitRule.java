...
public class SplitRule extends EmbeddingRule {
    ...
    public final boolean checkParentBounds$window_release(float density, Rect bounds) {
        ...
        return (this.minWidthDp == 0 || width >= convertDpToPx(density, this.minWidthDp)) && (this.minHeightDp == 0 || height >= convertDpToPx(density, this.minHeightDp)) && (this.minSmallestWidthDp == 0 || Math.min(width, height) >= convertDpToPx(density, this.minSmallestWidthDp)) && (height < width ? Intrinsics.areEqual(this.maxAspectRatioInLandscape, EmbeddingAspectRatio.ALWAYS_ALLOW) || (((((float) width) * 1.0f) / ((float) height)) > this.maxAspectRatioInLandscape.getValue() ? 1 : (((((float) width) * 1.0f) / ((float) height)) == this.maxAspectRatioInLandscape.getValue() ? 0 : -1)) <= 0 : Intrinsics.areEqual(this.maxAspectRatioInPortrait, EmbeddingAspectRatio.ALWAYS_ALLOW) || (((((float) height) * 1.0f) / ((float) width)) > this.maxAspectRatioInPortrait.getValue() ? 1 : (((((float) height) * 1.0f) / ((float) width)) == this.maxAspectRatioInPortrait.getValue() ? 0 : -1)) <= 0);
    }
